    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="deleteGoodsById(props.item.id)">
    ...
    methods: {
      ...
      deleteGoodsById(goodsId){
        ...
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          alert("删除成功！");
          // 根据品牌信息删除商品分类
          this.$http.get("/item/spu/deleteById/" + goodsId)
            .then(({data}) => {
              //{data} :这里属于vue的高级部分,解构表达式. 等价于res.data
      ...
      upOrDown(saleable,spuId){
        console.log("上下架状态:"+saleable+":--:spuId:"+spuId);
        let name="上架";
        if (saleable != '1'){
          name="下架"
        }
        this.$message.confirm('是否'+name+'?', '提示', {
          ...
          //alert(name+"成功！");
          // 根据品牌信息删除商品分类
          this.$http.get("/item/spu/upOrDown/" + saleable+"/"+spuId)
            .then(({data}) => {
              //{data} :这里属于vue的高级部分,解构表达式. 等价于res.data
              this.$message.info(name+'成功!');
              this.getDataFromServer();
            }).catch((error)=>{
            this.$message.info(name+'操作失败!');
          })
        }).catch(()=>{
